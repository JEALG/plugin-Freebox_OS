...
class Free_CreateTil
{
    ...
    {
        log::add('Freebox_OS', 'debug', '┌── :fg-success:' . (__('Début de création des commandes pour', __FILE__)) . ' ::/fg: ' . $logicalinfo['homeadaptersName'] . ' ──');
        log::add('Freebox_OS', 'debug', '[WARNING] - ' . (__('Création équipement', __FILE__)) . ' : Home Adapters');
        ...
        log::add('Freebox_OS', 'debug', '[  OK  ] - ' . (__('Création équipement', __FILE__)) . ' : Home Adapters');
    }
    ...
    private static function createTil_debug($Free_API, $logicalinfo, $templatecore_V4)
    {
        ...
        log::add('Freebox_OS', 'debug', '[WARNING]');
        log::add('Freebox_OS', 'debug', '[WARNING] - LOG DEBUG : ' . 'TILES / NODES');
        log::add('Freebox_OS', 'debug', '[INFO] - LOG POUR DEBUG : ' . 'NODES');
        $Free_API->universal_get('universalAPI', null, null, 'home/nodes');
        log::add('Freebox_OS', 'debug', '[INFO] - LOG POUR DEBUG : ' . 'TILES');
        $Free_API->universal_get('tiles');
        log::add('Freebox_OS', 'debug', '[INFO]- LOG POUR DEBUG : ' . 'CAMERA');
        $Free_API->universal_get('universalAPI', null, null, 'camera');
        log::add('Freebox_OS', 'debug', '[INFO] - LOG POUR DEBUG : ' . 'HOME ADAPTERS');
        $Free_API->universal_get('universalAPI', null, null, 'home/adapters');
        log::add('Freebox_OS', 'debug', '[  OK  ] - FIN LOG DEBUG : ' . 'TILES / NODES');
            ...
            foreach ($result as $Equipement) {
                ...
                if ($_eq_category === true) {
                    ...
                    if ($_eq_type != 'camera' && $boucle_num != 2) {
                        ...
                        $Tile = Freebox_OS::AddEqLogic(($Equipement['label'] != '' ? $Equipement['label'] : $_eq_type), $_eq_node, $category, true, $_eq_type,  $_eq_action, null, $_autorefresh, 'default', null, $eq_group);
                    } else {
                    ...
                    foreach ($_eq_data as $Command) {
                        ...
                        if ($Command['label'] != '' &&  $_eq_category === true) {
                            ...
                            if ($_eq_type == 'camera' && $boucle_num != 2 && method_exists('camera', 'getUrl')) {
                                ...
                                //log::add('Freebox_OS', 'debug', '[  OK  ] - FIN CREATION POUR LES CAMERAS');
                                continue;
                            }
    ...
    private static function search_setting_string($_Eq_action, $Access, $Name, $_Eq_type = null, $Label_O = null, $eq_group = null, $_Cmd_ep_id = null)
    {
        $Setting1 = $_Eq_type;
        $Setting2 = $Name;
        $Setting3 = null;
    ...
    private static function search_setting_bool($_Eq_action, $Access, $Name, $_Eq_type, $Label_O = null, $eq_group = null, $_Cmd_ep_id = null, $Templatecore_V4 = null)
    {
        $Setting1 = $_Eq_type;
        $Setting2 = $Name;
        $Setting3 = null;
