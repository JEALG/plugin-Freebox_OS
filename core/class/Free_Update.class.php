...
class Free_Update
{
    public static function UpdateAction($logicalId, $logicalId_type, $logicalId_name, $logicalId_value, $logicalId_conf, $logicalId_eq, $_options, $_cmd)
    {
        ...
        switch ($update) {
            ...
            case 'LCD':
                if ($logicalId != 'refresh') {
                    Free_Update::update_LCD($logicalId, $logicalId_type, $logicalId_eq, $Free_API, $_options, $_cmd);
                }
                Free_Refresh::RefreshInformation($logicalId_eq->getId());
                break;
            case 'homeadapters':
    ...
    private static function update_LCD($logicalId, $logicalId_type, $logicalId_eq, $Free_API, $_options, $_cmd)
    {
        $_enabled = true;
        ...
        switch ($logicalId) {
            ...
            case "WakeonLAN":
                ...
                if ($_options['mac_address'] == null) {
                    log::add('Freebox_OS', 'error', (__('Adresse mac vide', __FILE__)));
                    break;
                }
                $option = array(
            ...
            case "add_del_mac":
                ...
                if ($_options['ip'] == null || $_options['mac_address'] == null) {
                    ...
                    log::add('Freebox_OS', 'error', (__('IP ou adresse mac vide', __FILE__)));
                    break;
                }
                $option = array(
                    "mac" => $_options['mac_address'],
    ...
    private static function update_default($logicalId, $logicalId_type, $logicalId_eq, $Free_API, $_options, $_cmd, $logicalId_conf)
    {
        switch ($logicalId_type) {
            case 'slider':
                $_slider = (int) $_options['slider'];
                if ($_cmd->getConfiguration('invertslide') == 1) {
                    log::add('Freebox_OS', 'debug', '│ :fg-info:' . (__('Option Etat inverser curseur Active', __FILE__)) . ':/fg:');
                    $_maxvalueRAW = $_cmd->getConfiguration('maxValue');
                    $_maxvalue = (int) $_cmd->getConfiguration('maxValue');
                    $_minvalue = (int) $_cmd->getConfiguration('minValue');
                    //if (empty($_maxvalueRAW) && empty($_minvalueRAW)) {
                    if ($_maxvalueRAW === null || $_maxvalueRAW === '') {
                        log::add('Freebox_OS', 'ERROR', (__('La valeur maxi et mini de la commande ne doit pas être vide', __FILE__)) . ' : ' . (__('Max', __FILE__)) . ' = ' . $_maxvalue  . ' / ' . (__('Min', __FILE__)) . ' = ' . $_minvalue);
                        $parametre['value'] = $_slider;
                    } else {
                        log::add('Freebox_OS', 'Debug', '│ :fg-info:' . (__('Les valeurs', __FILE__)) . ' : ' . (__('Max', __FILE__)) . ' =:/fg: ' . $_maxvalue  . ' / :fg-info:' . (__('Min', __FILE__)) . ' =:/fg: ' . $_minvalue);
                        $parametre['value'] = ($_maxvalue - $_minvalue) - $_slider;
                    }
            ...
            default:
                ...
                if ($logicalId_conf >= 0 && (stripos($logicalId, 'PB_On') !== FALSE || stripos($logicalId, 'PB_Off') !== FALSE)) {
                    ...
                    log::add('Freebox_OS', 'debug', '│ ' . (__('Paramétrage spécifique BP ON/OFF', __FILE__)) . ' (' . $logicalId . ' avec Id ' . $logicalId_conf . ') : ' . $parametre['value']);
                    ...
                    if (stripos($logicalId, 'PB_UP') !== false || stripos($logicalId, 'PB_DOWN') !== false) {
                        ...
                        log::add('Freebox_OS', 'debug', '│ ' . (__('Paramétrage spécifique BP UP/DOWN ', __FILE__)) . ' (' . $logicalId . ' ' . (__('Récupération ID', __FILE__)) . ' ' . (__('avec ID', __FILE__)) . $logicalId_conf . $logicalId_conf . ') : ' . $parametre['value']);
                        $logicalId = $logicalId_conf;
                    } else {
                        $parametre['value'] = true;
